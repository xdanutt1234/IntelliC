<script lang="ts">
 import { onMount } from "svelte";
  ...
  const handleClickOutside = (event) => {
    if (isDropdownOpen && !event.target.closest('.language-dropdown') && !event.target.closest('.dropdown-content')) {
      isDropdownOpen = false;
    }
  };
...
<div class="problem-menu" style="position: fixed; left: 0; top: 0px; bottom: 0; height: 100%; width: 10%; overflow-y: auto; background-color: #333; color: #fff; padding: 10px;">
        ...
        {#if isDropdownOpen}
          <div class="dropdown-content">
            {#each languages as languageObj}
              <button on:click={() => changeLanguage(languageObj)}>{languageObj.label}</button>
            {/each}
          </div>
        {/if}
      </div>
